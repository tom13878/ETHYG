...
# load packages
library(pacman)
p_load(rprojroot, frontier, moments, dplyr, qdap, ggplot2, AER, stargazer)
...
# GYGA variables
# CHECK RootDepth has 0 values for >150 obs
GYGA <- c("GGD", "AI", "TS")

# environmental and all dummy variables
dummies <- c("dumoxen", "noN", "impr", "crop_count2") 

# soil variables
soil <- c("SOC2", "phdum_gt70", "phdum55_2_70")

# variables explaining technical inefficiency.
zvars <- c("-1", "age", "ed_any", "sex",
           "title",
           "credit",
           "extension",
           "dist_market",
...
# Spec with soil and GYGA variables
TL_form_environ_GYGA_soil <- paste(TL_form_environ_GYGA,
                              paste(soil, collapse=" + "),
                              sep=" + ")
...
TL_form_fullz2 <- paste(deparse(formula(TL_form_environ_GYGA_soil), width.cutoff = 500),
                       paste(zvars, collapse=" + "), sep=" | ")
...
TL4 <- lm(TL_form_environ_GYGA_soil, data=db1)

stargazer(CD1, CD2, TL1, TL2, TL3, TL4, type = "text")
...
sfaTLGYGA_soil <- sfa(TL_form_environ_GYGA_soil, data=db1)

summary(sfaTLenviron)
summary(sfaTLGYGA)
summary(sfaTLGYGA_soil)
...
sfaTL_z2 <- sfa(TL_form_fullz2, data=db1)
