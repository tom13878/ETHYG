...
Don't use lograin. This basically stands in place of an intercept.
Don;t use both the phdummies together as they are highly correlated

```{r eval=FALSE}
modl <- lm(logN ~ SOC2, data=db1[db1$yesN ==1, ])
plot(db1$SOC2, db1$logN)
abline(modl)

modl2 <- lm(logyld ~ SOC2, data=db1[db1$yesN ==1, ])
plot(db1$SOC2, db1$logyld)
abline(modl2)

# we see that having SOC2 in the model does very little to the
# coefficient on logN. But it does pull on the intercept.

modl3 <- lm(logyld ~ logN, data=db1[db1$yesN == 1, ])
plot(db1$logN, db1$logyld)
abline(modl3)

modl4 <- lm(logyld ~ logN + SOC2, data=db1[db1$yesN == 1, ])
plot(db1$logN, db1$logyld)
abline(modl4)
```
