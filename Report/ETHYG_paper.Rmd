...
```{r read_data}
dataPath <- "D:/Data/Projects/ETHYG/"
db1 <- readRDS(file.path(dataPath, "Cache/db1.rds"))
```
...
```{r echo=FALSE}
#stargazer(as.data.frame(db1), type = "text", digits=2, out=".\\FigTab\\summaryStat.html")
dataPath <- "D:/Data/Projects/ETHYG/"
db1 <- readRDS(file.path(dataPath, "Cache/db1.rds"))
CD1 <- lm(logyld ~ logN, data=db1)
