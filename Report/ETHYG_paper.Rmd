...
knitr::opts_chunk$set(fig.path = 'FigTab/', echo = FALSE, warning = FALSE, message = FALSE)
```
...
knitr::kable(xtable, caption = "check")
``` 
...
```{r summary_stat, results='asis', message=FALSE, echo=FALSE}
stargazer(db9, header = FALSE)
```

```{r GYGA_LSMS, fig.cap = "GYGA and LSMS. \\label{figurelabel1}", fig.height=8, fig.width=8, fig.pos="H"}
GYGA_LSMS
```


```{r ZONE_LSMS, fig.cap = "ZONE and LSMS. \\label{figurelabel2}", fig.height=8, fig.width=8}
ZONE_LSMS
```

# Results

```{r Waterfall, fig.cap = "Waterfall plot. \\label{figurelabel3}", fig.height=8, fig.width=8, fig.pos="H"}
waterfall
```
